...
# Convert labels to binary format: 0 for benign, 1 for SQLi
df['label'] = df['classification'].apply(lambda x: 1 if str(x).strip().lower() == 'anomalous' else 0)
...
def clean_url(text):
    ...
    text = re.sub(r'[\W_]+', ' ', text)                # Remove special characters and underscores
...
# 5. Define the model (BERT + CNN + BiLSTM)
...
# 6. Training
y_train_tensor = tf.convert_to_tensor(y_train, dtype=tf.float32)  # Convert to TensorFlow tensor
model.fit(
    {'input_ids': X_train_ids, 'attention_mask': X_train_masks},
    y_train_tensor,
    epochs=1,                      # Start small
    batch_size=8,                  # Lower batch size for CPU
    verbose=1
)

# 7. Evaluation
y_test_tensor = tf.convert_to_tensor(y_test, dtype=tf.float32)  # Convert to TensorFlow tensor
